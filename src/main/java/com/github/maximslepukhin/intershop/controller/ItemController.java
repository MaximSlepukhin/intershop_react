...
public class ItemController {
    ...
    public Mono<String> showItems(@RequestParam(defaultValue = "") String search,
        ...
        return itemService.findItems(sort, search, search)
                .skip(skip)
                .take(pageSize + 1)
                .collectList()
                .flatMap(items -> {
                    ...
                    return itemService.toItemsWithCount(Flux.fromIterable(trimmedItems), cart)
                            .collectList()
                            .map(itemsWithCount -> {
                                ItemWithCount[][] split = itemService.splitToRows(itemsWithCount, ELEMENTS_IN_ROWS);
                                ItemsResult paging = new ItemsResult(hasNext, pageNumber, pageSize);
                                model.addAttribute("items", split);
